...
class AlarmClockOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_alarm_advanced(
        ...
        if user_input is not None:
            ...
            if coordinator:
                ...
                new_alarm = AlarmData(
                    ...
                    max_snooze_count=alarm_data.get(CONF_MAX_SNOOZE_COUNT, DEFAULT_MAX_SNOOZE_COUNT),
                    auto_dismiss_timeout=alarm_data.get(CONF_AUTO_DISMISS_TIMEOUT, DEFAULT_AUTO_DISMISS_TIMEOUT),
                    pre_alarm_duration=alarm_data.get(CONF_PRE_ALARM_DURATION, DEFAULT_PRE_ALARM_DURATION),
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                ...
                vol.Optional(
                    CONF_MAX_SNOOZE_COUNT,
                    default=DEFAULT_MAX_SNOOZE_COUNT
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(min=0, max=10, step=1)
                ),
                vol.Optional(
                    CONF_AUTO_DISMISS_TIMEOUT,
                    default=DEFAULT_AUTO_DISMISS_TIMEOUT
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                ...
                vol.Optional(
                    CONF_PRE_ALARM_DURATION,
                    default=DEFAULT_PRE_ALARM_DURATION
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
    ...
    async def async_step_manage_alarms(
        ...
        alarm_options = [
            selector.SelectOptionDict(
                value=alarm_id,
                label=f"{alarm.data.name} ({alarm.data.time})"
            )
    ...
    async def async_step_edit_alarm(
        ...
        if user_input is not None:
            ...
            if isinstance(time_value, dict):
                alarm.data.time = f"{time_value.get('hours', 0):02d}:{time_value.get('minutes', 0):02d}"
            else:
                alarm.data.time = str(time_value)

            alarm.data.days = user_input.get(CONF_DAYS, alarm.data.days)
            alarm.data.snooze_duration = user_input.get(CONF_SNOOZE_DURATION, alarm.data.snooze_duration)
            alarm.data.max_snooze_count = user_input.get(CONF_MAX_SNOOZE_COUNT, alarm.data.max_snooze_count)
            alarm.data.auto_dismiss_timeout = user_input.get(CONF_AUTO_DISMISS_TIMEOUT, alarm.data.auto_dismiss_timeout)
            alarm.data.pre_alarm_duration = user_input.get(CONF_PRE_ALARM_DURATION, alarm.data.pre_alarm_duration)
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                ...
                vol.Optional(
                    CONF_SNOOZE_DURATION,
                    default=alarm.data.snooze_duration
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=1, max=60, step=1, unit_of_measurement="minutes"
                    )
                ...
                vol.Optional(
                    CONF_MAX_SNOOZE_COUNT,
                    default=alarm.data.max_snooze_count
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(min=0, max=10, step=1)
                ),
                vol.Optional(
                    CONF_AUTO_DISMISS_TIMEOUT,
                    default=alarm.data.auto_dismiss_timeout
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=1, max=180, step=1, unit_of_measurement="minutes"
                    )
                ...
                vol.Optional(
                    CONF_PRE_ALARM_DURATION,
                    default=alarm.data.pre_alarm_duration
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=0, max=60, step=1, unit_of_measurement="minutes"
                    )
    ...
    async def async_step_global_settings(
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                vol.Optional(
                    CONF_WATCHDOG_TIMEOUT,
                    default=current_settings.get(CONF_WATCHDOG_TIMEOUT, DEFAULT_WATCHDOG_TIMEOUT)
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                ...
                vol.Optional(
                    CONF_MISSED_ALARM_GRACE_PERIOD,
                    default=current_settings.get(CONF_MISSED_ALARM_GRACE_PERIOD, DEFAULT_MISSED_ALARM_GRACE_PERIOD)
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=1, max=60, step=1, unit_of_measurement="minutes"
                    )
